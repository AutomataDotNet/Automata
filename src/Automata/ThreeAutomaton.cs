...
namespace Microsoft.Automata
{
    public class ThreeAutomaton<S> : IAutomaton<S> 
    {
        ...
        ThreeAutomaton<S> MinimizeClassical(IBooleanAlgebra<S> solver, int timeout)
        {
            ...
            Dictionary<Pair<int, int>, S> combinedConditionMap = new Dictionary<Pair<int, int>, S>();
            foreach (int state in fa.States)
            {
            ...
            foreach (var kv in combinedConditionMap)
                mfaTransitions.Add(Move<S>.Create(kv.Key.First, kv.Key.Second, kv.Value));
        ...
        private bool AreDistinguishable(ThreeAutomaton<S> fa, Equivalence E, Pair<int, int> pq, IBooleanAlgebraPositive<S> solver)
        {
            foreach (Move<S> from_p in fa.GetMovesFrom(pq.First))
                foreach (Move<S> from_q in fa.GetMovesFrom(pq.Second))
