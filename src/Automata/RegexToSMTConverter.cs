...
namespace Microsoft.Automata
{
    public class RegexToSMTConverter
    {
        ...
        private string GetSMTRanges(IList<Pair<uint, uint>> ranges)
        {
            string res = "";
            if (ranges.Count == 0)
                res = string.Format("(as re-empty-set (RegEx {0}))", CHAR);
            if (ranges.Count == 1)
                res = string.Format("(re-range {0} {1})", EscapeCharSMT((char)ranges[0].First), EscapeCharSMT((char)ranges[0].Second));
            else
            {
                for (int i = 0; i < ranges.Count; i++)
                {
                    ...
                    res += string.Format("(re-range {0} {1})", EscapeCharSMT((char)ranges[i].First), EscapeCharSMT((char)ranges[i].Second));
                }
