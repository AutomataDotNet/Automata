...
namespace Microsoft.Automata.Internal.DirectedGraphs
{
    public static class DgmlWriter
    {
        ...
        public static void AutomatonToDgml<S>(int k, IAutomaton<S> fa, string name, System.IO.TextWriter tw, Func<S,string> describeS = null)
        {
            ...
            var nonFinalMoves = new Dictionary<Pair<int, int>, List<S>>();
            ...
            foreach (var move in fa.GetMoves())
            {
                ...
                else if (!isfinal(move.Label))
                {
                    ...
                    List<S> rules;
                    var p = new Pair<int, int>(move.SourceState, move.TargetState);
            ...
            foreach (var move in nonFinalMoves)
                tw.WriteLine(GetNonFinalRuleInfo(k, fa, faf, move.Key.First, move.Key.Second, move.Value, describeS));
